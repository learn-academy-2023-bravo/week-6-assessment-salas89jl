...
// Return each element(object) in array make a new object with object's key=name capitalized
    // Split the name value in an array of words exp object.name = ["ford", "prefect"] toUppercase the first letter of each word
    // Join the array of words into a string ('')
    // Return the joined altered array with properties of occupation to saty '${name} is ${occupatoin}.'
// 
...
describe("sortRemain",() => {
  ...
  it("filter the integers from an array, and retrive remainder after dividing by three", () => {
    expect(sortRemain(hodgepodge1)).toEqual([ 2, 0, -1, 0 ])
    expect(sortRemain(hodgepodge2)).toEqual([ 2, 1, -1 ])
  })
} )
...
//   ● sortRemain › filter the integers from an array, and retrive remainder after dividing by three

//     ReferenceError: sortRemain is not defined

// b) Create the function that makes the test pass.

// Pseudo Code:
// 1. Create a function named sortRemain
// 2. Input will be an array of mixed media
// 3. copy filtered array into new a variable called numArray where values that are only integers.
...
// 4. Use .map to cycle through each value and do a 1expo3 or num * num * num
